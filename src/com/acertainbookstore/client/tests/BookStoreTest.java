...
public class BookStoreTest {
	...
	/**
	 * Test if a rating is done correctly.
	 * @throws BookStoreException
	 */
	@Test
	public void testRateOfBook() throws BookStoreException {
		Set<BookRating> bookRatesVals = new HashSet<BookRating>();
		bookRatesVals.add(new BookRating(TEST_ISBN, RATE));

		client.rateBooks(bookRatesVals);
		List<StockBook> listBooks = storeManager.getBooks();
		StockBook bookInList = listBooks.get(0);
		assertTrue(bookInList.getTotalRating() == RATE);
		assertTrue(bookInList.getTimesRated() == 1);
	...
	public void testTopRatedBooks() throws BookStoreException {
		Set<StockBook> booksToAdd = new HashSet<StockBook>();
		booksToAdd.add(new ImmutableStockBook(3333, "Bla", "blub", (float) 55, NUM_COPIES, 0, 1, 10, false));
		booksToAdd.add(new ImmutableStockBook(4444, "Bla", "blub", (float) 55, NUM_COPIES, 0, 2, 10, false));
		booksToAdd.add(new ImmutableStockBook(5555, "Bla", "blub", (float) 55, NUM_COPIES, 0, 3, 10, false));
		booksToAdd.add(new ImmutableStockBook(6666, "Bla", "blub", (float) 55, NUM_COPIES, 0, 4, 10, false));
		booksToAdd.add(new ImmutableStockBook(7777, "Bla", "blub", (float) 55, NUM_COPIES, 0, 5, 10, false));
		booksToAdd.add(new ImmutableStockBook(8888, "Bla", "blub", (float) 55, NUM_COPIES, 0, 6, 10, false));

		storeManager.addBooks(booksToAdd);

		// The top 2 books (3333 + 4444)
		List<Book> top2ratedBooks = client.getTopRatedBooks(2);
		assertEquals(top2ratedBooks.size(), 2);

		// Check if the expected ones a really the ones you get
		assertTrue(top2ratedBooks.get(0).getISBN() == 3333 );
		assertTrue(top2ratedBooks.get(1).getISBN() == 4444 );


	}

	/**
	 * Tests if a invalid rate is recognized.
	 * @throws BookStoreException
	 */
	@Test(expected=BookStoreException.class)
	public void testRateOfBookInvalidRate() throws BookStoreException {
		Set<BookRating> bookRatesVals = new HashSet<BookRating>();
		bookRatesVals.add(new BookRating(TEST_ISBN, -3));
