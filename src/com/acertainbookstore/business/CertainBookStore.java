...
public class CertainBookStore implements BookStore, StockManager {
	...
	public synchronized List<Book> getTopRatedBooks(int numBooks)
		...
		Collections.sort(listRatedBooks, new Comparator<BookStoreBook>() {
			@Override
			public int compare(BookStoreBook o1, BookStoreBook o2) {
				return Float.compare((((BookStoreBook) o2).getAverageRating()), (((BookStoreBook) o1).getAverageRating()));
			}
		});
	...
	public synchronized void rateBooks(Set<BookRating> bookRating)
		...
		for (BookRating bookRatingArg : bookRating) {
			book = bookMap.get(bookRatingArg.getISBN());
			book.addRating(bookRatingArg.getRating());
